...
class AudioFile:
    ...
    def filter(self, loFthresh, hiFthresh, Athresh) -> NDArray:
        return AudioFile.filtersignal(self.magspec,
                                      loFthresh,
                                      hiFthresh,
                                      Athresh)
    ...
    def windowedPeaks(self, numberFundamentalsInWindow: int, percentile: float) -> NDArray:
        ...
        signal = AudioFile.filtersignal(self.magspec, fund-50, loPass, 0)
        ...
        for i in range(numberWindows):
            ...
            tempPeaks = AudioFile.staticfindPeaks(signal[window], percentile=10, height=0, distance=fund//10)
    ...
    def printAggregateError(directory: str, numberOfFundamentalsInWindow: int, percentile: float, badData: list = None, SpecificType: str = None) -> None:
        nameArray = AudioFilesList(Path(directory))
        ...
        with open(f"AggError-{SpecificType}-{numberOfFundamentalsInWindow}-{percentile}.txt", "a") as f:
            ...
            f.write(f'{AudioFile.roundEntries(fileName=f"AggError-{SpecificType}-{numberOfFundamentalsInWindow}-{percentile}.txt", roundingValue=1)}')
